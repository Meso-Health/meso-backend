...
RSpec.describe "Provider Members", type: :request do
  ...
  describe "PATCH /members/:id" do
    ...
    context 'when the request is sent as multipart/form-data' do
      ...
      it 'updates the sent attributes as well as photo a member' do
        expect do
        ...
        expect(response).to be_successful
        expect(json.keys).to include(*%w[id household_id created_at absentee card_id full_name gender age phone_number fingerprints_guid photo_url membership_number medical_record_number])
        expect(json.fetch('id')).to eq member.id
