...
RSpec.describe Household, type: :model do
  ...
  describe '#merge!' do
    ...
    context 'when passed attributes to overwrite' do
      ...
      it 'copies over attachments correctly' do
        current = create(:household, photo: File.open(Rails.root.join("spec/factories/households/photo1.jpg")))
        other = create(:household, photo: File.open(Rails.root.join("spec/factories/households/photo2.jpg")))

        current.merge!(other, %w[photo_id])

        expect(current.photo_id).to eq other.photo_id
      end
    end
  end
