PaperTrail.without_versioning do
  ...
  include UgandaDataGenerator

  # Create admin divisions, providers, billables, and diagnoses
  UgandaDataGenerator.setup_initial_data
  ...
  hc1 = Provider.where(name: 'Rwibaale Health Center').first
  hc2 = Provider.where(name: 'Kyenjojo Health Center').first
  hospital1 = Provider.where(name: 'Fort Portal Hospital').first

  country = AdministrativeDivision.where(name: 'Uganda').first
  region = AdministrativeDivision.where(name: 'Western Region').first
  district = AdministrativeDivision.where(name: 'Kyenjojo').first
  village1 = AdministrativeDivision.where(name: 'Rwibaale').first
  subvillage1 = AdministrativeDivision.where(name: 'Kabwera').first

  # Create users
  create(:user, :system_admin, username: 'system_admin', password: 'password', name: 'Admin', administrative_division: region)
  demo_enroller = create(:user, :enrollment, username: 'demo_enroller', password: 'password', name: 'Demo Enroller', administrative_division: village1)

  # Create enrollment periods
  create(:enrollment_period, start_date: 2.years.ago, end_date: 1.years.ago, administrative_division: district)
  create(:enrollment_period, start_date: 6.months.ago, end_date: 6.months.from_now, administrative_division: district)

  # Create households, members, enrollment records and payments
  [[subvillage1, demo_enroller]].each do |village, enroller|
    # Need to renew
    generate_ugandan_household(village, enroller, household_size: 2, paying: true, renewed: false, include_member_photos: true)

    # Need to renew, with beneficiaries
    generate_ugandan_household(village, enroller, household_size: 5, paying: true, renewed: false, include_member_photos: true)

    # Does not need to renew
    generate_ugandan_household(village, enroller, household_size: 5, paying: true, renewed: true, include_member_photos: true)
  end
  ...
  [[hc1, 'hc1', 'Rwibaale Health Center'],
   [hc2, 'hc2', 'Kyenjojo Health Center']].each do |provider, username, name|
